...
test_files = os.listdir(eval_data_path)

x_te = torch.zeros(len(test_files), T, dtype=torch.float)
indices = []

for k, f in enumerate(test_files):
    basename = f.split('.')[0]

    indices.append(basename)

    # Load the audio signal and normalize it.
    _, x = wavfile.read(os.path.join(eval_data_path, f))
    x = np.asarray(x, dtype='float')
    x /= np.max(np.abs(x))
...
# coefficients (after adding a small constant to make sure nothing blows up
# when scattering coefficients are close to zero).
Sx_te = torch.log(torch.abs(Sx_te) + log_eps)

###############################################################################
# Finally, we average along the last dimension (time) to get a time-shift
# invariant representation.
Sx_te = torch.mean(Sx_te, dim=-1)
